...
/* Updated includes matching the new naming convention */
#include "sr_types.h"
#include "sr_config.h"
 ...
 *
 * Configures SysTick to generate interrupts based on SR_TICK_RATE_HZ
...
/**
 * @brief Check if a task has experienced a stack overflow.
 *
 * Inspects the deep bounds of a task's stack to ensure the canary/magic
 * words have not been overwritten by deep execution runtime contexts.
 *
 * @param taskHandle Handle of the task to verify.
 *
 * @retval sRTOS_OK Stack is perfectly safe.
 * @retval sRTOS_ERROR Stack overflow signature detected!
 */
...
{
  return timeoutMS * (SR_TICK_RATE_HZ / 1000);
}
